...
TEST_USERS: list[dict[str, str]] = [
    ...
    {"email": "test+08@loupe.app", "name": "Hannah Schmidt", "archetype": "lorcana"},
    {
        ...
        "archetype": "mixed_high_end",
    },
    {
        "email": "test+10@loupe.app",
        "name": "Riley Anderson",
        "archetype": "mixed_beginner",
    },
]
...
# Grade distribution per archetype (avg grade x portfolio size)
PROFILES: dict[str, dict] = {
    "vintage_pokemon": {
        "avg_grade": 8.5,
        "size": 12,
        "house_mix": [("psa", 0.7), ("loupe", 0.3)],
    },
    "modern_pokemon": {
        "avg_grade": 9.2,
        "size": 18,
        "house_mix": [("psa", 0.5), ("loupe", 0.5)],
    },
    "magic_reserved": {
        "avg_grade": 7.8,
        "size": 8,
        "house_mix": [("bgs", 0.6), ("psa", 0.4)],
    },
    "yugioh_meta": {
        "avg_grade": 8.8,
        "size": 15,
        "house_mix": [("psa", 0.6), ("cgc", 0.4)],
    },
    "sports_basketball": {
        "avg_grade": 9.0,
        "size": 14,
        "house_mix": [("psa", 0.8), ("bgs", 0.2)],
    },
    "sports_baseball": {
        "avg_grade": 8.7,
        "size": 16,
        "house_mix": [("psa", 0.7), ("sgc", 0.3)],
    },
    "onepiece": {
        "avg_grade": 9.3,
        "size": 10,
        "house_mix": [("loupe", 0.7), ("cgc", 0.3)],
    },
    "lorcana": {"avg_grade": 9.1, "size": 9, "house_mix": [("loupe", 1.0)]},
    "mixed_high_end": {
        "avg_grade": 9.5,
        "size": 25,
        "house_mix": [("psa", 0.6), ("bgs", 0.3), ("cgc", 0.1)],
    },
    ...
    for archetype, spec in CATALOG.items():
        ...
        for name, number, rarity, year, image_url in spec["cards"]:
            existing_card = (
                await db.execute(
                    select(Card).where(
                        Card.set_id == cset.id, Card.name == name, Card.number == number
                    )
                )
            ).scalar_one_or_none()
...
async def seed_user(
    db, *, email: str, name: str, archetype: str, catalog: dict[str, list[Card]]
) -> User:
    ...
    if existing_grades:
        logger.info(
            "  %d graded cards already exist for %s; skipping portfolio",
            len(existing_grades),
            email,
        )
        return user
...
async def main() -> None:
    ...
    async with sessionmaker() as db:
        ...
        total_value = sum((g.estimated_value_usd or Decimal("0")) for g in total_grades)
        print()
