...
async def test_google_sign_in_creates_user(client, monkeypatch):
    ...
    resp = await client.post("/v1/auth/google", json={"id_token": "fake-token-1234567890"})
    assert resp.status_code == 200, resp.text
...
async def test_refresh_token_rotation(client, monkeypatch):
    ...
    first = await client.post("/v1/auth/apple", json={"identity_token": "fake-token-1234567890"})
...
async def test_refresh_rejects_invalid(client):
    resp = await client.post(
        "/v1/auth/refresh", json={"refresh_token": "a" * 50}
    )
