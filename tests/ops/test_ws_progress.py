...
@pytest.mark.asyncio
async def test_ws_prices_rejects_invalid_token():
    from starlette.testclient import TestClient
    ...
    app = create_app()
    with TestClient(app) as tc, pytest.raises(Exception):  # closed with 1008
        with tc.websocket_connect("/ws/prices?token=garbage"):
            pass


@pytest.mark.asyncio
async def test_ws_prices_hello_frame(created_user):
    from starlette.testclient import TestClient
    ...
    with TestClient(app) as tc, tc.websocket_connect(f"/ws/prices?token={token}") as ws:
        hello = ws.receive_json()
        assert hello["type"] == "hello"
        assert hello["data"]["user_id"] == str(created_user.id)
